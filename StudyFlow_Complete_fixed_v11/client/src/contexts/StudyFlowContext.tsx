  ...
  async function loadUserData(uid: string) {
    ...
    try {
      ...
      if (setsError && setsError.code !== 'PGRST116') {
        // PGRST116 = no row yet, expected for a brand-new user. Anything
        // else (missing column, permissions, etc.) is worth surfacing.
        toast.error('Could not load your settings: ' + setsError.message);
      }
      if (sets) {
        setSettings({
          user_id: uid,
          ...
          avatar_url: sets.avatar_url ?? '',
          theme: sets.theme ?? 'dusk',
          font_size: sets.font_size ?? 'medium',
          profile_setup_done: sets.profile_setup_done ?? false,
        });
      } else {
    ...
    const { error } = await sb.from('user_settings').upsert(
      {
        ...
        profile_setup_done: s.profile_setup_done,
